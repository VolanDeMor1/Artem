...
module.exports = {
	name: 'front',
	category: "",
	description: 'Просмотр скина (спереди) игрока Minecraft.',
    aliases: ["перед", "fr"],
	availability: true,
	run: async (message, args, bot) => {
        ...
        request(searchuuid, function(err, response, body){
            if(!err){
                ...
                let embed = new Discord.MessageEmbed()
                .setAuthor(`Перед ${suuid.data.player.username}`, `https://visage.surgeplay.com/face/${suuid.data.player.id}`)
                .setColor('#2f3136')
                .setDescription(`[Скачать скин](https://visage.surgeplay.com/skin/${suuid.data.player.id})`)
                .setImage(`https://visage.surgeplay.com/front/${suuid.data.player.id}`)
